...
class Ball(pygame.sprite.Sprite):
    def __init__(self, x,y,dx,dy):
        ...
        self.dx = dx
        self.dy = dy
        self.stop = 0 #counts number of times ball has stoped for scoring purposes
        self.go = False
        self.redirect = False; #used to trigger calculation of ball collide 
    def move(self):
        self.x += self.dx
        self.y += self.dy
    ...
    '''
        ...
        if(((abs(self.dx) > stop_dx)and(abs(self.dy )> stop_dy))and((abs(self.dx) > 0.05)and(abs(self.dy) > 0.05))):
            ...
            self.dx = 0
        '''
    def calcCollide(self, line):
        m = findSlope(line)#slope of the barrier line
        x = ((self.y + (1/m)*self.x)-(line[0][1]- m*line[0][0]))/(m+(1/m))
        y = x*m+(line[0][1]- m*line[0][0])
        return [x,y]
        perp_dist = dist([self.x,self.y],[x,y])
        print "perp_dist: ", perp_dist
        m2 = self.dy/self.dx #slope of the ball
        x = ((self.y + (m2)*self.x)-(line[0][1]- m*line[0][0]))/(m - m2)
        y = x*m+(line[0][1]- m*line[0][0])
        vector_dist = dist([self.x,self.y],[x,y])
        
        distance = (vector_dist/perp_dist)*self.r
        print "DISTANCE: " , vector_dist
        collision_coords = [x + (x-self.x)*distance/vector_dist, y + (y-self.y)*distance/vector_dist]
...
def runGame():
    ...
    for n in range(len(ball_collec)):
        if(ball_collec[0].go):
            drawSquare(collision)
        if(ball_collec[n].redirect):
            for line in lines:
                collision = ball_collec[n].calcCollide(line)
                print collision
            
            #if(ball_collec[n].Check(line)):#if within boundary
             #   ball_collec[n].pointToLine(line)
        ball_collec[n].redirect = False
    pygame.display.update()
